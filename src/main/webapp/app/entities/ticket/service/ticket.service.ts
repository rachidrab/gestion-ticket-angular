...
export class TicketService {
  ...
  protected convertDateFromClient(ticket: ITicket): ITicket {
    return Object.assign({}, ticket, {
      dateOuverture: ticket.dateOuverture?.isValid() ? ticket.dateOuverture.format(DATE_FORMAT) : undefined,
      dateCloture: ticket.dateCloture?.isValid() ? ticket.dateCloture.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((ticket: ITicket) => {
        ticket.dateOuverture = ticket.dateOuverture ? dayjs(ticket.dateOuverture) : undefined;
        ticket.dateCloture = ticket.dateCloture ? dayjs(ticket.dateCloture) : undefined;
      });
    }
    return res;
  }
